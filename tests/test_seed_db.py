...
def test_seed_imports_legacy_irom_header(tmp_path, monkeypatch):
    csv_path = tmp_path / "nutrition.csv"
    csv_path.write_text(
        "name,serving_size,calories,protein,carbohydrate,fat,fiber,sugars,sodium,calcium,potassium,vitamin_c,vitamin_a,irom,caffeine,water,cholesterol,alcohol\n"
        "Spinach,100 g,23,2.9,3.6,0.4,2.2,0.4,79,99,558,28.1,469,2.71,0,91.4,0,0\n",
        encoding="utf-8",
    )

    test_db_path = tmp_path / "seed_test_irom.db"
    engine = create_engine(
        f"sqlite:///{test_db_path}",
        connect_args={"check_same_thread": False}
    )
